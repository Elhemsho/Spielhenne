...
const TournamentState = {
  active:       false,
  scores:       [0, 0],        // [p1, p2]
  roundHistory: [],            // { game, winner }
  gameOrder:    [],            // shuffled GAMES array
  currentRound: 0,
  currentGame:  null,
  mathScores:   [0, 0],
  mathPhase:    0,             // 0 = p1 playing, 1 = p2 playing, 2 = done
};
...
// ----------------------------------------------------------------
const T_GAMES = [
  { id: 'connectfour', emoji: '🔴', de: 'Vier Gewinnt',      en: 'Connect Four',   url: 'connectfour.html' },
  { id: 'battleship',  emoji: '🚢', de: 'Schiffe versenken', en: 'Battleship',     url: 'battleship.html'  },
  { id: 'memory',      emoji: '🃏', de: 'Memory',            en: 'Memory',         url: 'memory.html'      },
  { id: 'yazy',        emoji: '🎲', de: 'Kniffel',           en: 'Yazy',           url: 'yazy.html'        },
  { id: 'mathquiz',    emoji: '➗', de: 'Mathe Quiz',        en: 'Math Quiz',      url: 'mathquiz.html'    },
];

// ----------------------------------------------------------------
// 3. TEXTS (both languages)
// ----------------------------------------------------------------
const T_LANG = {
  de: {
    ...
    mq_pts:      'Punkte', mq_ok: 'OK',
    mq_correct:  '✓ Richtig!', mq_wrong:  '✗ Falsch!',
    mq_done:     'Zeit! Punkte:',
  ...
  en: {
    ...
    mq_pts:      'Points', mq_ok: 'OK',
    mq_correct:  '✓ Correct!', mq_wrong: '✗ Wrong!',
    mq_done:     'Time! Points:',
    history_draw:'Draw',
...
// ----------------------------------------------------------------
function saveTS() {
  localStorage.setItem('t_state', JSON.stringify(TournamentState));
}
function loadTS() {
  try {
    const raw = localStorage.getItem('t_state');
    if (!raw) return false;
...
// Show one panel inside the overlay, hide others
...
// ----------------------------------------------------------------
// 6. SCOREBOARD (fixed top banner during tournament)
// ----------------------------------------------------------------
function tUpdateScoreboard() {
  ...
  const dots = TournamentState.roundHistory.map((r,i) => {
    const cls = r.winner === 1 ? 'p1' : r.winner === 2 ? 'p2' : 'draw';
    return `<span class="t-sb-dot t-sb-dot--${cls}"></span>`;
  }).join('') + Array(5 - TournamentState.roundHistory.length).fill('<span class="t-sb-dot"></span>').join('');
...
function tBuildIntro() {
  ...
  panel.innerHTML = `
    ...
    <div class="t-game-chips">
      ${T_GAMES.map(g=>`<span class="t-chip">${g.emoji} ${tGameName(g)}</span>`).join('')}
    </div>
...
function tRunSpin(targetGame, onDone) {
  ...
  panel.innerHTML = `
    ...
    <div id="t-spin-result" style="display:none">
      <div class="t-spin-winner">${targetGame.emoji} ${tGameName(targetGame)}</div>
  ...
  // Build: 5 loops + target at end
  const items = [...T_GAMES,...T_GAMES,...T_GAMES,...T_GAMES,...T_GAMES, targetGame];
  strip.innerHTML = items.map(g=>`
    <div class="t-slot-item">${g.emoji} ${tGameName(g)}</div>
  `).join('');

  const itemH = 44;
  const centerOff = 22; // half item height → centers item 0
...
function tShowFinal() {
  ...
  if(s1>s2)      { crown='👑'; label=tx.champion;       winnerName=`<span class="t-col-p1">${tx.p1}</span>`; }
  else if(s2>s1) { crown='👑'; label=tx.champion;       winnerName=`<span class="t-col-p2">${tx.p2}</span>`; }
  ...
  const histHTML = TournamentState.roundHistory.map((r,i)=>{
    ...
    return `<div class="t-hist-row">
      <span>${r.game.emoji} ${tGameName(r.game)}</span>
  ...
  panel.innerHTML = `
    ...
    <div class="t-podium">
      <div class="t-podium-col">
        ...
        <div class="t-podium-bar t-podium-bar--p1" style="height:${h1}px">${s1>s2?'🏆':''}</div>
        <div class="t-podium-lbl">${tx.p1}</div>
      </div>
      <div class="t-podium-col">
        <div class="t-podium-score t-col-p2">${s2}</div>
        <div class="t-podium-bar t-podium-bar--p2" style="height:${h2}px">${s2>s1?'🏆':''}</div>
...
// ----------------------------------------------------------------
// 11. MATH QUIZ (embedded, no page redirect)
// ----------------------------------------------------------------
const TMQ = {
...
// ----------------------------------------------------------------
function tCelebrate() {
  // Use existing confetti if available
...
// ----------------------------------------------------------------
// 13. TOURNAMENT CONTROLLER
// ----------------------------------------------------------------
const Tournament = {

  open() {
    // Rebuild intro each time (language may have changed)
    tBuildIntro();
  ...
  launchGame() {
    ...
    if(game.id === 'mathquiz'){
      ...
      // Navigate to game page with tournament flag
  ...
  // Called by game pages when a game ends
  reportResult(winner) {
...
// ----------------------------------------------------------------
// 14. INJECT DOM (overlay + scoreboard)
// ----------------------------------------------------------------
function tInjectDOM() {
  if(tEl('t-overlay')) return; // already injected
  ...
  spans.forEach(span => {
    ...
    span.onclick = () => {
      if(TournamentState.active){
        // Resume – show scoreboard / current state
        tUpdateScoreboard();
        Tournament.open();
      } else {
        Tournament.open();
...
// ----------------------------------------------------------------
// 16. GAME PAGE INTEGRATION
// Auto-detect if we are on a tournament game page and show result button
// ----------------------------------------------------------------
function tCheckGamePage() {
  const params = new URLSearchParams(window.location.search);
  if(!params.get('tournament')) return;

  if(!loadTS()) return; // no active tournament

  // Show scoreboard on game page
  tUpdateScoreboard();
  const sb = tEl('t-scoreboard');
  if(sb) sb.style.display='flex';

  // Wait for the game's own win-overlay to appear, then add a "report result" button
  // We hook into the existing overlay mechanisms via MutationObserver
  const gameId = TournamentState.currentGame?.id;

  // Poll for champion overlay
  let reported = false;
  const observer = new MutationObserver(() => {
    ...
    if(mem && mem.style.display==='flex'){
      ...
      if(winner){ reported=true; observer.disconnect(); tInjectGameResultBtn(mem, winner); }
    }
    // Yazy
    ...
    // Battleship
    const bs = tEl('championOverlay');
    if(bs && bs.dataset.winner && !bs.classList.contains('hidden')){
      reported=true; observer.disconnect(); tInjectGameResultBtn(bs, bs.dataset.winner);
    }
  });
...
function tInjectGameResultBtn(container, winner) {
  ...
  btn.textContent = `🏆 ${tx.btn_next} → Olympiade`;
  btn.onclick = () => {
  ...
  // Try to insert inside winner box
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore active tournament state
  if(loadTS() && TournamentState.active){
    tUpdateScoreboard();
    tCheckGamePage();
  } else {
    tCheckGamePage(); // still check in case URL has ?tournament=1
  }
});

// Re-hook after global.js rebuilds the nav (language switch)
